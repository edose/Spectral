x011b_fit <- function(list.in, item.noise.mag = 0.0, image.noise.mag = 0.0, fix.seed=TRUE) {
...
#  df <- df[df$SecantZA <= 2.0,] # keep images only at Airmass 2 or less.
  ...
  # Capture and append to list the R Code generating raw data (this .R file)
    connection <- file("011b_fit.R")           # *this* code file as text
  ...
  save(list.out, file="011b_list")  # saves updated list as a file.
