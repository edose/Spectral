plot_002b <- function(plot.style="grey") {
  ...
  df <- df[df$Alt %in% c(90),] # only at zenith, otherwise same as 002a plot.
  p <- ggplot(df, aes(x=nm, y=Transm, group=Alt, color=Alt))
  p <- p + geom_line(size=0.4,color="darkorchid2")
  p <- p + xlab("wavelength (nm)")
  p <- p + ylab("Transmission   \nfraction   ")
  p <- p + choose.theme("grey") 
  # put here: any post-SAS-style tweaks.
  p
  ggsave(filename="002b_plot.svg", width=9, height=6, unit="in", dpi=200)
}
