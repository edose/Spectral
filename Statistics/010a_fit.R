x010a_fit <- function(list.in, item.noise.mag = 0.0, image.noise.mag = 0.0, fix.seed=TRUE) {
  ...
  # Capture and append to list the R Code generating raw data (this .R file)
    connection <- file("010a_fit.R")           # *this* code file as text
  ...
  save(list.out, file="010a_list")  # saves updated list as a file.
