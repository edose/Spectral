x012d_fit <- function(list.in, item.noise.mag = 0.0, image.noise.mag = 0.0, fix.seed=TRUE) {
  ...
  # Capture and append to list the R Code generating raw data (this .R file)
    connection <- file("012d_fit.R")           # *this* code file as text
  ...
  save(list.out, file="012d_list")  # saves updated list as a file.
